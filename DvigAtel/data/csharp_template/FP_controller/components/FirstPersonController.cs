...
#pragma warning restore CS0649
	...
	private void Init() 
	{
		...
		if (!camera || cameraMode == CameraMode.CREATE_AUTOMATICALLY)
		{
            ...
            camera.ViewportMask = camera.ViewportMask & ~(1 << 1);
        }
	...
	private void UpdateVelocity(float ifps, float updatePart)
	{
		...
		if (Input.MouseGrab)
		{
			worldTransform *= new Mat4(MathLib.Rotate(new quat(vec3.UP, -Input.MouseDeltaPosition.x * mouseSensitivity * updatePart)));
	...
	private void UpdateCamera()
	{
		...
		if (Input.MouseGrab)
		{
			...
			cameraVerticalAngle -= Input.MouseDeltaPosition.y * mouseSensitivity;
